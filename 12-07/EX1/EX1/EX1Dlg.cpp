...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_EDIT1, etxtSample);
	DDX_Control(pDX, IDC_LIST1, listSample);
	DDX_Control(pDX, IDC_BUTTON1, btnAdd);
	DDX_Control(pDX, IDC_BUTTON2, btnRemove);
}
...
BEGIN_MESSAGE_MAP(CEX1Dlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDOK, &CEX1Dlg::OnBnClickedOk)
...
void CEX1Dlg::OnBnClickedOk()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	//CDialogEx::OnOK();
	etxtSample.GetWindowText(etxtSampleText);
}
...
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	listSample.AddString(etxtSampleText);
}
...
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	int index = listSample.GetCurSel();
	if(index != LB_ERR) {
		listSample.DeleteString(index);
	}
}
